...
def train_and_evaluate(file_path, N, k, hidden_dim, num_epochs, batch_size,
    ...
    param_count = count_gru_params(input_dim, hidden_dim, num_layers=1)
    return test_loss, r2, vaf, param_count

################################################################################
#                  Experiment Loop Using the Output File as Checkpoint
################################################################################
if __name__ == "__main__":
    ...
    # Use the output file as the checkpoint
    ...
    start_config_index = num_completed // num_runs      # which parameter combination
    start_run_index = num_completed % num_runs            # which run for that combination

    print(f"Resuming from experiment #{num_completed + 1} "
          f"(config index {start_config_index}, run {start_run_index + 1}).")

    # Loop over parameter combinations starting from the one indicated by the checkpoint.
    for config_index in range(start_config_index, len(parameter_combinations)):
        ...
        # For the very first configuration to resume, start from the saved run index.
        ...
        for run in range(current_run_start, num_runs):
            ...
            print(f"Config {config_index+1}/{len(parameter_combinations)} "
            ...
            try:
                test_loss, r2, vaf, param_count = train_and_evaluate(
                ...
                # Save the updated results (which also serve as the checkpoint).
                with open(output_file, 'wb') as f:
                    pickle.dump(results, f)
                print(f"Experiment saved. ({len(results)}/{total_experiments})")
