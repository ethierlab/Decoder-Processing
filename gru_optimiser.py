...
def train_and_evaluate(file_path, N, k, hidden_dim, num_epochs, batch_size, learning_rate, train_split=0.8, cutoff=None, fs=None, order=5, device='cpu'):
    # Load and preprocess data
    train_dataset, test_dataset, test_raw, input_dim = load_data_and_preprocess(file_path, N, k, train_split, cutoff, fs, order)
    ...
    model = GRUModel(input_size=input_dim, hidden_size=hidden_dim, num_layers=1).to(device)
    ...
    return test_loss, r2, vaf

#######################################
# Running All Experiments with Incremental Saving
#######################################
def main():
    ...
    hidden_dims = [32, 64]        # Different hidden dimensions
    batch_sizes = [32, 64]        # Different batch sizes
    num_epochs_list = [500]       # You can vary this as needed
    learning_rates = [0.001]      # Different learning rates if desired
    ...
    for idx, (N, k, hidden_dim, batch_size, num_epochs, learning_rate) in enumerate(parameter_combinations, start=1):
        ...
        print(f"Running experiment {idx}/{total_experiments} with parameters: N={N}, k={k}, hidden_dim={hidden_dim}, batch_size={batch_size}, num_epochs={num_epochs}, learning_rate={learning_rate}")

        try:
            test_loss, r2, vaf = train_and_evaluate(
                file_path=file_path,
                N=N,
                k=k,
            ...
            print(f"Experiment {idx}/{total_experiments} with parameters N={N}, k={k}, hidden_dim={hidden_dim}, batch_size={batch_size}, num_epochs={num_epochs}, learning_rate={learning_rate} failed with error: {e}")
            # Optionally, you can choose to continue or break
            continue

    print(f"All experiments completed. Final results saved to {output_file}.")
