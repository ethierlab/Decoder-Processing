...
def process_combination(bin_size, smoothing_length, spike_times_list, duration, pca_results, umap_results, tsne_results):
    # Calculer le sigma pour le lissage
...
# Fonction pour exécuter le multiprocessing
def run_in_parallel(spike_times_list, duration, bin_sizes, smoothing_lengths):
    ...
    for bin_size in bin_sizes:
        for smoothing_length in smoothing_lengths:
            process = multiprocessing.Process(target=process_combination, args=(bin_size, smoothing_length, spike_times_list, duration, pca_results, umap_results, tsne_results))
    ...
    return dict(pca_results), dict(umap_results), dict(tsne_results)

# Exemple d'utilisation avec tes données
if __name__ == '__main__':
    # Charger les données et configurer les paramètres
    pkl_file = 'experiment_data.pkl'
    data = load_data(pkl_file)
    data_dict = data['data']
    
    unit_selection = 'both'  # À changer selon tes besoins
    ...
    smoothing_lengths = [0.01, 0.02, 0.05, 0.1]
    
    # Exécuter le multiprocessing
    pca_results, umap_results, tsne_results = run_in_parallel(spike_times_list, duration, bin_sizes, smoothing_lengths)
