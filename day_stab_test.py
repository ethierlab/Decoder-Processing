...
###############################################################################
# MAIN (CV temporel corrigé + compute MIG) — multi-seed
###############################################################################
def main():
    ...
    # seeds
    parser.add_argument('--seed', type=int, default=42, help='base seed (used if --seeds not provided)')
    parser.add_argument('--n_seeds', type=int, default=0, help='if >0, run seeds = [seed, seed+1, ..., seed+n_seeds-1]')
    parser.add_argument('--seeds', type=int, nargs='+', default=None, help='explicit list of seeds, e.g. --seeds 1 2 3')
    ...
    # Déterminer la liste des seeds à exécuter
    if args.seeds is not None and len(args.seeds) > 0:
        seed_list = list(dict.fromkeys(args.seeds))  # unique & preserve order
    elif args.n_seeds and args.n_seeds > 0:
        seed_list = list(range(args.seed, args.seed + args.n_seeds))
    else:
        seed_list = [args.seed]

    # Mode perf/repro
    global PERF_MODE, SEED
    if args.perf_mode:
        PERF_MODE = True
    ...
    base_splits = time_kfold_splits(X0_raw.shape[0], args.n_folds)
    ...
    for seed_val in seed_list:
        print(f"\n========== SEED {seed_val} ==========")
        # mettre à jour le SEED global pour worker_init_fn/generator
        SEED = seed_val
        set_seed(seed_val)

        for fold_idx, (val_start, val_end) in enumerate(base_splits):
            print(f"\n[Seed {seed_val}] [Fold {fold_idx+1}/{args.n_folds}] val=[{val_start}:{val_end})")
            ...
            if X_left_p.size and X_right_p.size:
                ...
                cuts_train = cuts_left + [c + len(X_left_p) for c in cuts_right] + [len(X_left_p)]  # frontière left/right
            elif X_left_p.size:
            ...
            # Fit manifold sur TRAIN uniquement (seed dépend de seed_val et du fold)
            dimred_model_day0 = get_dimred_model(X_train_time, args.dimred, max(N_PCA, 2), seed_val + fold_idx)
            ...
            for ch_idx, vaf_single in enumerate(vaf_ch_te):
                results.append({
                    "seed": seed_val,                  # <-- pour l'appariement seed×fold
                    "day": day0,
                    "day_int": 0,
                    "align": "crossval",
            ...
            for d_val in unique_days:
                ...
                dimred_model_dayX = get_dimred_model(X_proc, args.dimred, N_PCA, seed_val + fold_idx)
            ...
            # un petit nettoyage mémoire GPU entre folds
